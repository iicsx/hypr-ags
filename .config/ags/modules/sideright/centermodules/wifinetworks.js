          ...
          execAsync(["nmcli", "-g", "NAME", "connection", "show"])
            .then((savedConnections) => {
              ...
              if (!savedSSIDs.includes(connectAttempt)) {
                // SSID not saved: show password input
                if (networkAuth) {
                  networkAuth.revealChild = true;
                }
              } else {
                // If SSID is saved, hide password input
                if (networkAuth) {
...
const NetResource = (icon, command) => {
  ...
  const widget = Button({
    child: Box({
      ...
      children: [MaterialIcon(icon, "very-small"), resourceLabel],
      setup: (self) =>
        self.poll(2000, () =>
...
const CurrentNetwork = () => {
  ...
  const networkBandwidth = Box({
    ...
    children: [
      NetResource(
        "arrow_warm_up",
        `${App.configDir}/scripts/network_scripts/network_bandwidth.py sent`,
      ),
      NetResource(
        "arrow_cool_down",
        `${App.configDir}/scripts/network_scripts/network_bandwidth.py recv`,
      ),
    ],
  });
  ...
  const authHeader = Box({
    ...
    hpack: "fill",
    spacing: 10,
    children: [networkAuthSSID, cancelAuthButton],
  });
  ...
  const authEntry = Entry({
    ...
    onAccept: (self) => {
      ...
      execAsync(["nmcli", "connection", "delete", connectAttempt]).catch(
        () => {},
      ); // Ignore error if SSID not found
  ...
  const authBox = Box({
    ...
    children: [authEntry, authVisible],
  });
    ...
    onClicked: () => {
      execAsync(["nmcli", "-t", "-f", "ACTIVE,NAME", "connection", "show"])
        .then((output) => {
          ...
          if (activeSSID) {
            execAsync(["nmcli", "connection", "delete", activeSSID]).catch(
              (err) =>
                Utils.execAsync([
                  "notify-send",
                  "Network",
  ...
  const propertiesButton = Button({
    ...
    hexpand: true,
    onClicked: () => {
      Utils.execAsync("nmcli -t -f uuid connection show --active")
        .then((uuid) => {
          if (uuid.trim()) {
  ...
  const networkProp = Revealer({
    ...
    child: Box({
      ...
      homogeneous: true,
      children: [propertiesButton, forgetButton],
      setup: setupCursorHover,
    }),
    setup: (self) =>
      self.hook(Network, (self) => {
  ...
  networkAuth = Revealer({
    ...
    child: Box({
      ...
      vertical: true,
      children: [authHeader, authBox, authFailed],
    }),
    setup: (self) =>
      self.hook(Network, (self) => {
        execAsync(["nmcli", "-g", "NAME", "connection", "show"])
          .then((savedConnections) => {
            ...
            if (
              ...
              (Network.wifi.state == "need_auth" &&
                !savedSSIDs.includes(Network.wifi.ssid))
            ) {
              authLock = true;
      ...
      Overlay({
        ...
        child: Scrollable({
          ...
          child: Box({
            ...
            vertical: true,
            className:
              "spacing-v-5 sidebar-centermodules-scrollgradient-bottom-contentmargin",
