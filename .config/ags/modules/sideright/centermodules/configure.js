    ...
    onChange: (self, newValue) => {
      if (save)
        execAsync([
          "bash",
          "-c",
          `${App.configDir}/scripts/hyprland/hyprconfigurator.py \
            --key ${option} \
    ...
    onReset: async (self) => {
      if (save)
        exec(`bash -c '${App.configDir}/scripts/hyprland/hyprconfigurator.py \
                ...
                --reset \
                --file "\${XDG_CONFIG_HOME:-$HOME/.config}/hypr/custom/general.conf"'`);
      else exec("hyprctl reload");
      extraOnReset(self);
    },
  });

const HyprlandSpinButton = ({
  icon,
  name,
  desc = null,
  option,
  ...
  ConfigSpinButton({
    ...
    fetchValue: () =>
      Number(JSON.parse(exec(`hyprctl getoption -j ${option}`))["int"]),
    onChange: (self, newValue) => {
      if (save)
        execAsync([
          "bash",
  ...
  const mainContent = Overlay({
    ...
    child: Scrollable({
      ...
      child: Box({
        vertical: true,
        className:
          "spacing-v-10 sidebar-centermodules-scrollgradient-bottom-contentmargin",
        children: [
          ConfigSection({
